/**
 * This program implements a parallel code via a kernel call using atomic methods to do a matrix 
...
__global__ void MatrixMulAtomic(float* M, float* N, float* P, int Width);
...
int main(int argc, char *argv[]) {
    ...
    if (width < TILE_WIDTH) {
        printf("The matrix width should be equal to or larger than the tile size, which is 32 in this case: %s \n", argv[2]);
        return 3;
    }
    ...
    /* to warm up the GPU */
    MatrixMulAtomic<<<dimGrid, dimBlock>>>(dev_a, dev_b, dev_c, width);
